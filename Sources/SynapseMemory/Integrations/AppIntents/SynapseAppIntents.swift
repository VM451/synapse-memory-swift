...
public struct SearchMemoriesIntent: AppIntent {
    ...
    public func perform() async throws -> some IntentResult & ReturnsValue<[String]> {
        guard let client = SynapseClient.shared else {
            throw NSError(domain: "SynapseMemory.AppIntents", code: 1, userInfo: [NSLocalizedDescriptionKey: "SynapseClient shared instance not initialized"])
        }
...
public struct AddMemoryIntent: AppIntent {
    ...
    public static let description = IntentDescription("Stores a new factual memory item into SynapseMemory.")
