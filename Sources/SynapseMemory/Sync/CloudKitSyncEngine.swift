...
public actor CloudKitSyncEngine {
    ...
    private let logger = Logger(subsystem: "com.synapse.memory.swift", category: "CloudKitSyncEngine")
    
    public static let defaultZoneName = "SynapsePrivateZone"
    public static let subscriptionID = "synapse-db-changes"
    
    private var changeToken: CKServerChangeToken?
    
    public init(containerId: String? = nil) {
        self.containerId = (containerId != nil && !containerId!.isEmpty) ? containerId! : "iCloud.com.synapse.memory.swift"
    }
        ...
        return try await withCheckedThrowingContinuation { continuation in
            operation.recordWasChangedBlock = { recordID, result in
                if case .success(let record) = result {
                    if record.recordType == "SynapseMemory", let memory = MemoryItem.fromCKRecord(record) {
                        updatedMemories.append(memory)
                    } else if record.recordType == "SynapseEntity", let entity = Entity.fromCKRecord(record) {
                        updatedEntities.append(entity)
                    } else if record.recordType == "SynapseRelation", let relation = Relation.fromCKRecord(record) {
