...
SynapseMemory includes a local Knowledge Graph memory engine inspired by graphiti and Mem0. It extracts relational triples `(Subject, Relation, Object)` from conversation turns to maintain structured relationships alongside dense vector embeddings.
...
Knowledge graph entities and relations are automatically serialized into `SynapseEntity` and `SynapseRelation` `CKRecord` types and synchronized across the user's Apple devices using CloudKit private database subscriptions.
